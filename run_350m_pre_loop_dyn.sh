...
CUDA_VISIBLE_DEVICES=0,1,2,3 torchrun --nproc_per_node 4 --master_port=29500 torchrun_main.py \
    ...
    --optimizer adam \
    --grad_clipping 0.0 \
    --run_name "350m_res_${norm_type}_lr${learning_rates}_loop_${LOOP_STRATEGY}" \
    --save_dir "${LPAI_MODEL_DIR}/350m_res_${norm_type}_lr${learning_rates}_loop_${LOOP_STRATEGY}" \
    --loop_enabled