...
CUDA_VISIBLE_DEVICES=0,1,2,3 torchrun --nproc_per_node 4 --master_port=29503 torchrun_main.py \
    --model_config configs/llama_350m_qwen.json \
    --lr $learning_rates \
    --batch_size 32 \
    ...
    --optimizer adam \
    --grad_clipping 0.0 \
    --run_name "${LPAI_MODEL_DIR}/350m_res_${norm_type}_lr${learning_rates}_qwen" \
    --save_dir "${LPAI_MODEL_DIR}/350m_qwen_res_${norm_type}_lr${learning_rates}" \
    --max_length 1024